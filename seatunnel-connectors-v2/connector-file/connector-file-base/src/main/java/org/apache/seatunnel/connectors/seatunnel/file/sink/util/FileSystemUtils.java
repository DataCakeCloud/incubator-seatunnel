...
public class FileSystemUtils implements Serializable {
    ...
    public void deleteFiles(Path[] paths){
        for(int i=0;i<paths.length;i++){
            ...
            try {
                deleteFile(paths[i].toString());
            } catch (IOException e) {
                String message = String.format("删除文件[%s]时发生IO异常,请检查您的网络是否正常！",
                        paths[i].toString());
                log.error(message);
                throw new FileConnectorException(
                        CommonErrorCode.FILE_OPERATION_FAILED, "delete file " + paths[i] + " error",e);
            }
        }
    }
    ...
    public Path[] hdfsDirList(String dir){
        ...
        Path[] files = null;
        String filterFileName = "T_*";
        try {
            PathFilter pathFilter = new GlobFilter(filterFileName);
            FileStatus[] status = getFileSystem(dir).listStatus(path,pathFilter);
            files = new Path[status.length];
            for(int i=0;i<status.length;i++){
                files[i] = status[i].getPath();
            }
        } catch (IOException e) {
            String message = String.format("获取目录[%s]下文件名以[T-]开头的文件列表时发生网络IO异常,请检查您的网络是否正常！",
                    dir);
            log.error(message);
            throw new FileConnectorException(
                    CommonErrorCode.FILE_OPERATION_FAILED, "delete file " + dir + " error",e);
        }
        return files;
    }
