...
public abstract class AbstractWriteStrategy implements WriteStrategy {
    ...
    public void write(SeaTunnelRow seaTunnelRow) throws FileConnectorException {
        if (BaseSinkConfig.OVERWRITE_MODE.equalsIgnoreCase(fileSinkConfig.getMode())&& !deleteFile){
            String path = getTruncateDataDir(seaTunnelRow);
            try {
                if (fileSystemUtils.fileExist(path)){
                    Path[] paths = fileSystemUtils.hdfsDirList(path);
                    fileSystemUtils.deleteFiles(paths);
                }
            }catch (IOException e){
                throw new FileConnectorException(
                        CommonErrorCode.FILE_OPERATION_FAILED, "delete file " + path + " error",e);
    ...
    public String getTruncateDataDir(@NonNull SeaTunnelRow seaTunnelRow) {
        String location = fileSinkConfig.getPath();
        List<Integer> partitionFieldsIndexInRow = fileSinkConfig.getPartitionFieldsIndexInRow();
        if(CollectionUtils.isEmpty(partitionFieldsIndexInRow)){
            return location;
        }else{
            ...
            if (location.endsWith("/")){
                return location+beingWrittenFileKey;
            }else {
                return location+"/"+beingWrittenFileKey;
            }
        }
