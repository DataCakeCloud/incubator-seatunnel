...
public class SourceConfig extends CommonConfig {
    ...
    public SourceConfig(Config pluginConfig) {
        ...
        if (pluginConfig.hasPath(READ_PARTITIONS.key())) {
            List<String> partitions = pluginConfig.getStringList(READ_PARTITIONS.key());
            List<Expression> partitionFilters = new ArrayList<>();
            for (String partition : partitions) {
                String key = partition.split("=", 2)[0];
                String value = partition.split("=", 2)[1];
                partitionFilters.add(Expressions.equal(key,value));
            }
            filter = buildAndExpression(partitionFilters);
        }
    }
    ...
    public static Expression buildAndExpression(List<Expression> expressions) {
        if (expressions.size() == 1) {
            ...
            Expression firstTwoAnd = Expressions.and(expressions.get(0), expressions.get(1));
            List<Expression> remaining = expressions.subList(2, expressions.size());
            return buildAndExpression(Arrays.asList(firstTwoAnd, buildAndExpression(remaining)));
        }
    }
