...
public class JdbcSinkWriter implements SinkWriter<SeaTunnelRow, XidInfo, JdbcSinkState> {
    ...
    public JdbcSinkWriter(
        ...
        this.outputFormat =
                new JdbcOutputFormatBuilder(dialect, connectionProvider, jdbcSinkConfig, rowType)
                        .build();
        this.postSql = jdbcSinkConfig.getPostSql();
    }
    ...
    public void close() throws IOException {
        ...
        if(postSql != null && postSql.size() > 0){
            ...
            connection = outputFormat.getConnection();
            JdbcUtil.executeSqls(connection,postSql);
            LOG.info(String.format("execute presql %s success", StringUtils.join(postSql,",")));
        }
        outputFormat.close();
    }
}
