...
public class FlinkCommandArgs extends AbstractCommandArgs {
    ...
    @Parameter(
            names = {"-gss", "--get-source-schema"},
            description = "Get source connection schema!")
    private boolean gss = false;

    @Parameter(
            names = {"-gts", "--get-target-schema"},
            description = "Get target connection schema!")
    private boolean gts = false;

    @Parameter(
            names = {"-gsd", "--get-source-demo"},
            description = "Get source demo schema!")
    private boolean gsd = false;

    @Parameter(
            names = {"-gst", "--get-source-tables"},
            description = "Get source tables!")
    private boolean gst = false;

    @Parameter(
            names = {"-ct", "--createTable"},
            description = "Create source tables!")
    ...
    public Command<?> buildCommand() {
        ...
        if (gss) {
            return new GetSourceSchemaCommand(this);
        }
        if (gsd) {
            return new GetSourceSampleCommand(this);
        }
        if (gst) {
            return new GetSourceTableCommand(this);
        }
        if (ct) {
