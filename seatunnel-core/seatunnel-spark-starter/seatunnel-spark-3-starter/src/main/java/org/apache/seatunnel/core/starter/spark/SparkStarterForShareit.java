...
public class SparkStarterForShareit implements Starter {
    ...
    private SparkStarterForShareit(String[] args, SparkCommandArgs commandArgs) {
        this.args = args;
        this.commandArgs = commandArgs;
    }

    public static void main(String[] args) throws IOException {
        String[] args1 = new String[]{"-e","cluster","--objectStoragePath","/Users/shareit/seatunnel-tools/apache-seatunnel-incubating-2.3.1", "--configStr",""};
        SparkStarterForShareit starter = getInstance(args);
     ...
     */
    public static SparkStarterForShareit getInstance(String[] args) {
    ...
    protected List<String> buildFinal() {
        ...
        appendOption(commands, "--configStr", this.commandArgs.getConfigStr());
    ...
    protected void appendPaths(List<String> commands, String option, List<Path> paths) {
        if (!paths.isEmpty()) {
            String values = "";
            if (StringUtils.isNoneEmpty(commandArgs.getObjectStoragePath())){
                values = assembleJarPath(commandArgs.getObjectStoragePath(), paths ,commandArgs.getConfigFile());
            } else {
                values = paths.stream().map(Path::toString).collect(Collectors.joining(","));
            }
            appendOption(commands, option, values);
        }
    }

    private String assembleJarPath(String objectStoragePath, List<Path> paths, String configFile) {
        String values = paths.stream().map(Path::toString).filter(data -> data.contains("apache-seatunnel-incubating-2.3.1")).map(path ->
                objectStoragePath + path.split("apache-seatunnel-incubating-2.3.1")[1]
        ).collect(Collectors.joining(","));
        return values;
    }
    ...
    protected void appendAppJar(List<String> commands) {
        commands.add(commandArgs.getObjectStoragePath() + "/starter/" + EngineType.SPARK3.getStarterJarName());
    }
    ...
    private static class ClientModeSparkStarter extends SparkStarterForShareit {
    ...
    private static class ClusterModeSparkStarter extends SparkStarterForShareit {
